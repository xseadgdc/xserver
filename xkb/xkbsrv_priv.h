...
Bool SrvXkbVirtualModsToReal(XkbDescPtr xkb, unsigned int virtua_mask,
...
Status SrvXkbChangeKeycodeRange(XkbDescPtr xkb, int minKC, int maxKC, XkbChangesPtr changes);
void XkbFreeInfo(XkbSrvInfoPtr xkbi);
Status SrvXkbChangeTypesOfKey(XkbDescPtr xkb, int key, int nGroups, unsigned int groups,
                              int *newTypesIn, XkbMapChangesPtr changes);
int SrvXkbKeyTypesForCoreSymbols(XkbDescPtr xkb, int map_width, KeySym *core_syms,
...
/* key latching */
int SrvXkbLatchModifiers(DeviceIntPtr pXDev, CARD8 mask, CARD8 latches);
int SrvXkbLatchGroup(DeviceIntPtr pXDev, int group);
