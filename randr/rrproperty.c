...
ProcRRListOutputProperties(ClientPtr client)
{
    ...
    if (numProps) {
        ...
        if (client->swapped) SwapLongs(pAtoms, numProps);
    }

    WriteToClient(client, sizeof(xRRListOutputPropertiesReply), &rep);
    WriteToClient(client, sizeof(pAtoms), pAtoms);
...
ProcRRQueryOutputProperty(ClientPtr client)
{
    ...
    if (prop->num_valid) {
        ...
        if (client->swapped) SwapLongs((CARD32*)extra, prop->num_valid);
    }
 ...
 */
    ...
    if (len) {
        ...
        switch (rep.format) {
        case 32:
            if (client->swapped) SwapLongs((CARD32*)extra, len / sizeof(CARD32));
            break;
        case 16:
